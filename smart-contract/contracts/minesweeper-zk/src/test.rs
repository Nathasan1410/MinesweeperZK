...
// Unit tests for the minesweeper-zk contract using a simple mock GameHub.
...
use crate::{Error, MinesweeperZkContract, MinesweeperZkContractClient};
...
fn setup_test() -> (
    Env,
    MinesweeperZkContractClient<'static>,
    ...
    // Deploy minesweeper-zk with admin and GameHub address
    let contract_id = env.register(MinesweeperZkContract, (&admin, &hub_addr));
    let client = MinesweeperZkContractClient::new(&env, &contract_id);

    // Register minesweeper-zk as a whitelisted game (mock does nothing)
...
fn test_upgrade_function_exists() {
    ...
    // Deploy minesweeper-zk with admin
    let contract_id = env.register(MinesweeperZkContract, (&admin, &hub_addr));
    let client = MinesweeperZkContractClient::new(&env, &contract_id);
